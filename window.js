...
SimpleChat.scrollToEnd = function (template) {
    ...
    template.$(".direct-chat-messages")
        .animate({
            scrollTop: template.$('.scroll-height')
                .height(),
        }, 300);
    template.$('.direct-chat-messages')
        .trigger('scroll');
...
Template.SimpleChatWindow.onCreated(function () {
    ...
    Meteor.call("SimpleChat.join", this.getRoomId(), this.getUsername(), this.getAvatar(), this.getName(), (err, res) => {
        if (err) {
            console.log(err);
        }
    });
...
Template.SimpleChatWindow.onRendered(function () {
    ...
    this.autorun(() => {
        if (this.subscriptionsReady()) {
             ...
             */
            this.$('.direct-chat-messages')
                .scrollTop(this.$('.scroll-height')[0].scrollHeight - this.scroll);
    ...
    if (this.showViewed) {
            ...
            if (window.visivility == "visible") {
                $('.notViewed')
                    ...
                    .each(function (i, o) {
                        ...
                        Meteor.call("SimpleChat.messageViewed", $(o)
                            .attr('id'), username, function (err) {
                            if (err) {
                                $(o)
                                    .addClass('notViewed');
                            }
        ...
        $(window)
            .on('resize scroll focus', checkViewed);
        $('.direct-chat-messages')
            .on('scroll', checkViewed);
    }
    $(window)
        .on('SimpleChat.newMessage', (e, id, doc) => {
            if (this.endScroll) {
            ...
            } else if (this.beep && username != doc.username) {
                new Audio('/packages/cesarve_simple-chat/assets/bell.mp3').play();
            }
        });
});
...
Template.SimpleChatWindow.helpers({
    ...
    simpleChats: function () {
        ...
        const chats = Chats.find({roomId: template.getRoomId()}, {sort: {date: 1}});

        const handleChanges = chats.observeChanges({
            added: (id, doc) => {
                ...
                if (!template.subscribing) {
                    $(window)
                        .trigger('SimpleChat.newMessage', [id, doc]);
                }
            },
        });
    ...
    viewedMe: function () {
        return Template.instance()
            .getUsername() == this.username || _.contains(this.viewedBy, Template.instance()
            .getUsername());
    },
    hasMore: function () {
        return Chats.find({
            roomId: Template.instance()
                .getRoomId(),
        }, {
            sort: {date: 1},
            limit: Template.instance()
                .limit
                .get(),
        })
            .count() === Template.instance()
            .limit
            .get();
    },
    me: function () {
...
Template.SimpleChatWindow.events({
    ...
    'click button#message-send': function () {
        const template = Template.instance();
        const $message = template.$('#simple-chat-message');
